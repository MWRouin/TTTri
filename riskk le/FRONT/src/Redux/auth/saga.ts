...
function* login({ payload: { email, password }, type }: UserData): SagaIterator {
    try {
        ...
        // response.data.item1.role = "Admin"
...
function* signup({ payload: { firstname, lastname, email, password, telephone, addresse } }: any): SagaIterator {
    try {
        const response = yield call(signupApi, { firstname, lastname, email, password, telephone, addresse });
        const user = response.data;
        api.setLoggedInUser(user);
        setAuthorization(user.token);

        // Dispatch success for signup
